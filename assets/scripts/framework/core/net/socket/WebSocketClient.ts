...
export default class WebSocketClinet {
    ...
    private connectWebSocket(ip: string, port: string | null, protocol: Net.Type) {
        ...
        if(protocol == "wss"){
            ...
            protocol = "wss";
            this._protocol = "wss";
            let fullUrl = `${this._protocol}://${this._ip}`;
            if (CC_DEBUG) Log.d(this._tag, `initWebSocket : ${fullUrl}`);
            Log.d("__log__connect_server_wss:",fullUrl,protocol,Manager.wssCacertUrl);
            ...
            Log.d("__log__connect_server_ws:",fullUrl,protocol,Manager.wssCacertUrl.length);
    ...
    private __onMessage(event: MessageEvent) {
        // Log.d("WebSocketClinet __onMessage",event);
