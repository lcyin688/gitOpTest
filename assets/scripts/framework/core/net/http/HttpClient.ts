...
export class HttpPackage {
    ...
    private isCallErr:boolean = false;
    ...
    public sendEx(cb?: (data: any) => void, errorcb?: (errorData: Http.Error) => void) {
        this.retryCount++;
        Log.d("sendEx:",this.retryCount);
        this.handlerId = setTimeout(() => {
            this.sendEx(cb,errorcb);
        }, 3000);
        this.send((rsp) => {
