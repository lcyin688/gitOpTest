...
export class UpdateHandler implements UpdateHandlerDelegate {
    setBundleCount(num: number): void {
...
export class UpdateItem {
    ...
    private checkCb(event: any) {
        ...
        switch (event.getEventCode()) {
            ...
            case Update.Code.ALREADY_UP_TO_DATE:
                ...
                } else if (this.bundle == Config.BUNDLE_HALL) {
    ...
    private updateCb(event: any) {
        ...
        if (isUpdateFinished) {
            ...
            let sp = JSON.stringify(searchPaths);
            cc.sys.localStorage.setItem('HotUpdateSearchPaths', sp);
            Log.d("HotUpdateSearchPaths",sp);
        ...
        if (this.isMain) {
            if (isUpdateFinished) {
                ...
                if (event.getDownloadedFiles() > 0) {
                    ...
                    // }, 200);
                    // isRestartApp = true;
                    Manager.bundleExtend.isRestartApp = true;
                } else {
        ...
        if (isUpdateFinished) {
            ...
            Manager.updateManager._dowonLoadedCallback(this);
        }
