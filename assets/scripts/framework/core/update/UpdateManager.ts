...
export class UpdateManager {
    ...
    init(){
        if(!this.isBrowser){
            let p = jsb.fileUtils.getSearchPaths();
            p.unshift(Manager.updateManager.storagePath);
            // p.push(Manager.updateManager.storagePath + "assets/");
            jsb.fileUtils.setSearchPaths(p);
        }
        let updateLevel = HotfixConfig.UpdateLevel;
        this.mainBundles = Config.MIAN_PACK_INCLUDE;
        this.hotUpdateUrl = HotfixConfig.HOT_UPDATE_URL_LOCAL;
        this._webUrl = Config.WEB_URL_REMOTE;
        Log.d("updateLevel=",updateLevel);
        if(updateLevel == 0){
            this.isSkipCheckUpdate = true;
        }else if(updateLevel == 1){
            this.isSkipCheckUpdate = false;
            this.hotUpdateUrl = HotfixConfig.HOT_UPDATE_URL_LOCAL;
            this._webUrl = Config.WEB_URL_LOCAL;
        }else if(updateLevel == 2){
            this.isSkipCheckUpdate = false;
            this.hotUpdateUrl = HotfixConfig.HOT_UPDATE_URL_REMOTE;
            this._webUrl = Config.WEB_URL_REMOTE;
        }
    }
    ...
    get storagePath() {
        ...
        let cp = writablePath + "caches/";
        if (!jsb.fileUtils.isDirectoryExist(cp)){
            jsb.fileUtils.createDirectory(cp);
        }
        Log.d(`当前存储路径为为:${cp}`);
        return cp;
    }
    ...
    private _webUrl = Config.WEB_URL_REMOTE;
    ...
    public get webUrl(): string {
        Log.d(`当前web地址为:${this._webUrl}`);
        return this._webUrl;
    }

    public set webUrl(value) {
        this._webUrl = value;
    }
    ...
    private isSkipUpdate(item: UpdateItem) {
        Log.d("this.isBrowser:",this.isBrowser);
        if (this.isBrowser) {
            //预览及浏览器下，不需要有更新的操作
    ...
    dowonLoadGame(bundle:string):boolean {
        let item = Manager.entryManager.delegate.getEntryConfig(bundle);
        if(item == null){
            return false;
        }

        let items = Manager.bundleExtend.getRelyItems(bundle);
        if(items == null || items.length == 0){
            Manager.tips.showFromId("TS_UPDATE_1");
            return false;
        }
        for (let index = 0; index < items.length; index++) {
            items[index].justDownload = true;
            this.dowonLoad(items[index]);      
        }
        return true;
    }
    ...
    public _dowonLoadedCallback(item: UpdateItem) {
        let isHallItem = Manager.bundleExtend.setDownloaded(item);
        item.justDownload = false;
        if (isHallItem){
    ...
    private _dowonLoad(item: UpdateItem) {
        this.current = item;
        // Log.d("_dowonLoad:",item.bundle,item.justDownload)
        this.current.isUpdating = true;
        this.loadVersions(this.current).then((isOk) => {
            if (isOk) {
                item.isUpdating = false;
                Log.d("1_dowonLoad:",item.bundle,item.justDownload)
                let status = this.getStatus(item.bundle);
                if (status == Update.Status.UP_TO_DATE) {
                    if (item.bundle == Config.BUNDLE_HALL && this.isMd5Change(Update.MAIN_PACK)) {
                        //大厅已经是最新，需要检测主包是否有更新
                        ...
                        item.handler.onLoadBundle(item);
                        this._dowonLoadedCallback(item);
                    }
                } else {
    ...
    checkAllowUpdate(item: UpdateItem, code: number) {
        ...
        Log.d(`item.updateName：`,item.updateName+" "+JSON.stringify(this.preVersions));
        ...
        if (versionInfo == undefined || versionInfo == null) {
            ...
            if (versionInfo.md5 == md5) {
                ...
                if (item.bundle == Config.BUNDLE_HALL) {
                    ...
                    if (this.isMd5Change(Update.MAIN_PACK) || this.isMd5Change(Config.BUNDLE_HALL)) {
