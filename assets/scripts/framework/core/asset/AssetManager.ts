...
export class AssetManager {
    ...
    private _onLoadComplete(cache: Resource.CacheData, complete: (data: Resource.CacheData) => void, err: Error | null, data: cc.Asset | cc.Asset[] | fgui.UIPackage | null) {
        
        // Log.e("AssetManager._onLoadComplete",cache.info.url);
        cache.isLoaded = true;
        //添加引用关系
        let tempCache = cache;
        if (err) {
    ...
    public loadFairy(
        bundle: BUNDLE_TYPE,
        path: string,
        type: typeof cc.BufferAsset,
        onProgress: (finish: number, total: number, item: cc.AssetManager.RequestItem) => void,
        onComplete: (data: Resource.CacheData) => void): void {
        if (CC_DEBUG) {
            Log.d(`load bundle : ${bundle} path : ${path}`)
        }
        // Log.e("loadFairy onProgress:",onProgress);
        let cache = Manager.cacheManager.get(bundle, path);
        if (cache) {
            ...
            let res = Manager.releaseManger.get(bundle, path);
            if (res) {
                // Log.e("AssetManager._onLoadComplete1",path);
                this._onLoadComplete(cache, onComplete, null, res);
                return;
            }
            let _bundle = this.getBundle(bundle);
            if (!_bundle) {
                //如果bundle不存在
                let error = new Error(`${this.logTag} ${bundle} 没有加载，请先加载`);
                // Log.e("AssetManager._onLoadComplete2",path);
                this._onLoadComplete(cache, onComplete, error, null);
                return;
            }
            res = _bundle.get(path, type);
            if (res) {
                // Log.e("AssetManager._onLoadComplete3",path);
                this._onLoadComplete(cache, onComplete, null, res);
            } else {
                if (onProgress) {
                    // Log.e("AssetManager._onLoadComplete4",path,onProgress);
                    fgui.UIPackage.loadPackage(_bundle,path,onProgress,this._onLoadComplete.bind(this, cache, onComplete))
                } else {
                    // Log.e("AssetManager._onLoadComplete5",path);
                    fgui.UIPackage.loadPackage(_bundle,path,this._onLoadComplete.bind(this, cache, onComplete))
                }
            }
        }
    ...
    // private _onLoadFairyComplete(cache: Resource.CacheData, complete: (data: Resource.CacheData) => void, err: Error | null, data: fgui.UIPackage | null) {
    //     cache.isLoaded = true;
    //     //添加引用关系
    //     let tempCache = cache;
    //     if (err) {
