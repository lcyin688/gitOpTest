...
export class DebugView extends cc.Component {
    ...
    onLoad() {
        ...
        //GM命令
        this.bindEvent("gm",this.onGMCmdManager);
        this.doOther();
    }
    debug: cc.Node = null!;
    ...
    private onGMCmdManager(){
        let node = cc.find("gm_box",this.node);
        let eb = node.getComponent(cc.EditBox);
        if(eb){
            if(eb.string.length == 0){
                eb.string = "addmoney 1,99999999999999999";
            }
            let cmd = eb.string.trim();
            if(cmd.length > 0){
                ...
                if(gs != null){
                    ...
                    Log.d(`gm GameService is null`);
                }
            }else{
                Log.d(`gm EditBox input is null`);
            }
        }else{
            Log.d(`gm EditBox is null`);
        }
        Log.d(`-----------发送GM命令失败------------`);
    }
}
